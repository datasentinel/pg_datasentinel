...
pgds_log_tempfile(ErrorData *edata)
{
	...
	p = strrchr(edata->message, ' ');
	e->bytes = (p != NULL) ? (int64) strtoll(p + 1, NULL, 10) : 0;

	strlcpy(e->message, edata->message, PGDS_TEMPFILE_MSG_LEN);
...
pgds_emit_log_process(ErrorData *edata)
{
	...
	/*
	 * Keyword matching on edata->message only works when the server runs
	 * without message translation; translated locales will be silently missed.
	 */
...
_PG_init(void)
{
	...
	DefineCustomBoolVariable("pg_datasentinel.enable_pss_memory",
							 "Also computes PSS (Proportional Set Size) memory per backend by reading "
							 "/proc/<pid>/smaps_rollup once per backend. More accurate than RSS, "
							 "but can be expensive if hundreds of backends. "
							 "Displayed in the ds_stat_activity view.",
							 NULL,
